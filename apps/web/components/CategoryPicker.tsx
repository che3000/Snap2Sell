"use client";
import { categories, type CategoryTree } from "@/packages/product/categories";
export function CategoryPicker({ value, onChange }: {value:string;onChange:(v:string)=>void}) {
  ...
  for(let i=0;i<3 && Object.keys(tree).length;i++) {
  ...
  return <div className="category-picker">
    ...
    <p className="field-help">下拉選單依提供的分類清單建立；尚未提供的子分類可自行填寫。</p>
  </div>;
}
