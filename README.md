...
7. **選擇與儲存**：從三個名稱、三個描述與三個建議價格中選擇，仍可自行編輯；商品確認後按儲存，`/api/save` 會整理 AI 文案、保存草稿並送出學習回饋。

照片不能證明真偽、保固、庫存或交易承諾。手機代數不確定時，程式會保留 iPhone 系列候選，不直接把猜測代數當成確定型號。一般辨識不會自動確認商品；儲存時的 AI 文案整理仍須在身分確認與必要問答完成後執行。
...
- 名稱與描述候選由 `packages/listing/options.ts` 根據現有資料產生，是模板整理，**不會各自額外呼叫 OpenAI**；確認商品後的 AI 文案生成集中在儲存流程，避免使用者需要操作另一個生成入口。
...
| `POST /api/generate` | 舊版相容接口；目前前端不再直接呼叫 |
| `POST /api/save` | 儲存時生成／保存文案、草稿版本與 feedback，並觸發 learning queue |
...
預覽區「產生蝦皮 Console 程式」會將當下草稿快照轉成可複製的獨立 JavaScript。先在蝦皮新增商品頁選好類別，再於 Console 執行。支援名稱、描述、價格、庫存、主商品貨號及狀態；只填草稿有值的欄位，會覆寫對應現值。逐欄回報缺失或畫面值是否保留，不呼叫發布／儲存。類別、圖片、物流、規格與其他動態屬性仍需人工操作。

`packages/shopee/console.ts` 隔離資料匯出與瀏覽器 adapter，`apps/web/components/ShopeeConsoleExport.tsx` 提供預覽與複製。沒有金鑰、登入憑證或額外 API 呼叫。描述透過瀏覽器文字編輯指令輸入富文字區，若不支援或回讀不一致會提示人工處理；DOM 回讀成功不等於平台已儲存，請核對字數、驗證與互動後的保留狀態。欄位 selector 來自實測 DOM，蝦皮更新後可能需要調整。
