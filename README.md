...
設定 `TEST_LOGIN_USERNAME`、`TEST_LOGIN_PASSWORD`、`TEST_SESSION_SECRET`（至少32字元）、`OPENAI_API_KEY`、`OPENAI_MODEL` 後，主站使用簡單帳密登入及共用後端 AI。Sites audience 需允許公開載入登入頁；所有資料 API、AI、圖片仍要求伺服器驗證的簽章 HttpOnly session cookie，不信任外部使用者標頭。修改密碼會使既有 session 失效。

每次登入建立獨立測試空間，有效7天；重新整理維持同一空間，登出再登入會建立新空間。不是正式多人帳號管理系統；共用密碼應私下提供，登出前匯出需保留的資料。共用模式禁止測試者更改 AI 金鑰。
...
`packages/listing/options.ts` 從現有商品事實產生三種名稱與描述格式，前端點選後寫入原有欄位，可再編輯並隨草稿保存。這些是資料整理模板，不會額外呼叫 AI 或新增賣點。三種售價使用 BigGo 可比價格的 30/50/70 百分位；資料不足時顯示等待行情，沒有模擬價格。
